<div class="w-screen relative left-1/2 right-1/2 -mx-[50vw] bg-white">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="w-full px-4 md:px-8 lg:px-28 py-10 bg-[{{ $landing->getValueOrDefault('eligibility_bg_color', '#41b99f') }}]/10 rounded flex flex-col justify-start items-start gap-5">
            <div class="text-black text-base md:text-lg lg:text-xl font-normal font-sans">{{ $landing->getValueOrDefault('eligibility_intro', 'Les propriétaires qui respectent ces conditions peuvent obtenir la prise en charge de leur installation :') }}</div>
            <div class="w-full flex flex-col justify-start items-start gap-2.5">
                <div class="text-black text-base md:text-lg lg:text-xl font-normal font-sans">{{ $landing->getValueOrDefault('eligibility_point1', '✔ Avoir plus de 70€ de facture mensuelle') }}</div>
                <div class="text-black text-base md:text-lg lg:text-xl font-normal font-sans">{{ $landing->getValueOrDefault('eligibility_point2', '✔ Vivre dans une commune éligible') }}</div>
                <div class="text-black text-base md:text-lg lg:text-xl font-normal font-sans">{{ $landing->getValueOrDefault('eligibility_point3', '✔ Etre propriétaire de sa maison') }}</div>
                <div class="text-black text-base md:text-lg lg:text-xl font-normal font-sans">{{ $landing->getValueOrDefault('eligibility_point4', '✔ Avoir la validation d\'un expert RGE') }}</div>
            </div>
            <div
                class="h-14 md:h-15 px-8 py-2.5 bg-[{{ $landing->getValueOrDefault('eligibility_btn_color', '#41b99f') }}] rounded border border-[{{ $landing->getValueOrDefault('eligibility_btn_color', '#41b99f') }}] flex justify-center items-center gap-3">
                ...
                <a href="{{ $landing->getValueOrDefault('eligibility_btn_url', route('form')) }}" class="text-right text-white text-lg font-bold font-sans uppercase">{{ $landing->getValueOrDefault('eligibility_btn_text', 'vérifier mes droits') }}</a>
                <svg width="24" height="25" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
